...
def test_linear_s8w_s8a_s8b_s8o():
    ...
    for i in range(13):
        ...
        sq_linear = W8A8B8O8Linear.from_float(linear, x_scale, y_scale).cuda()

        my_linear = cl.Linear_S8W_S8A_S8B_S8O_Unsecure(sq_linear)
...
def test_linear_s8w_s8a_s8b_s8o_slalom():
    timer = st.SingletonTimer(False)

    B = random.randint(1, 1)
    M = 16**random.randint(2, 2)
    N = 16**random.randint(2, 2)
    for i in range(13):

        linear = torch.nn.Linear(M*3, N*3, bias=True)

        x = torch.randn(B, 59, M*3)
        x_scale = x.abs().max() / 127
        qx = (x / x_scale).round().to(torch.int8)
        qx2 = qx.clone().detach()
        ...
        unsecure_linear = cl.Linear_S8W_S8A_S8B_S8O_Unsecure_Gpu(sq_linear)
        slalom_linear = cl.Linear_S8W_S8A_S8B_S8O_Slalom(sq_linear)

        t = timer.start(tag='Unsecure', category='Unsecure', exclude = i < 3)
        qx = qx.to(torch.device('cuda:0'))
        qy = unsecure_linear(qx)
        torch.cuda.synchronize()
        qy = qy.to(torch.device('cpu'))
        timer.end(t)
        t = timer.start(tag='Slalom', category='Slalom', exclude = i < 3)
        qy2 = slalom_linear(qx2)
        torch.cuda.synchronize()
        timer.end(t)

        print(qy)
        print(qy2)
        is_equal = torch.equal(qy, qy2)
...
if __name__ == '__main__':
    test_linear_s8w_s8a_s8b_s8o_slalom()
