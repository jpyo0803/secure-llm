...
processFromSmem(uint *regM, uint *regN, uint *threadResults, const uint *As,
                ...
                const uint threadRowInWarp, const uint threadColInWarp, uint64_t mod) {
      ...
      for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
        ...
        for (uint resIdxM = 0; resIdxM < TM; ++resIdxM) {
          for (uint resIdxN = 0; resIdxN < TN; ++resIdxN) {
            uint64_t tmp = static_cast<uint64_t>(threadResults[(wSubRowIdx * TM + resIdxM) * (WNITER * TN) +
                          (wSubColIdx * TN) + resIdxN]) + static_cast<uint64_t>(regM[wSubRowIdx * TM + resIdxM]) * static_cast<uint64_t>(regN[wSubColIdx * TN + resIdxN]);
            threadResults[(wSubRowIdx * TM + resIdxM) * (WNITER * TN) +
                          (wSubColIdx * TN) + resIdxN] = static_cast<uint>(tmp & (mod - 1));
                //         threadResults[(wSubRowIdx * TM + resIdxM) * (WNITER * TN) +
...
__global__ void __launch_bounds__(NUM_THREADS)
    sgemmWarptiling(uint *A, uint *B, uint *C, int M, int N, int K, uint64_t mod) {
  {
  ...
  for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
    ...
    wt::processFromSmem(regM, regN, threadResults, As, Bs, warpRow, warpCol,
                            threadRowInWarp, threadColInWarp, mod);
