...
namespace wt {
__device__ void loadFromGmem(int N, int K, const char *A, const char *B,
                             char *As, char *Bs, int innerRowA, int innerColA,
                             int innerRowB, int innerColB) {
  for (uint offset = 0; offset + rowStrideA <= BM; offset += rowStrideA) {
    const char4 tmp = reinterpret_cast<const char4 *>(
...
__device__ void
processFromSmem(int *regM, int *regN, int *threadResults, const char *As,
                const char *Bs, const uint warpRow, const uint warpCol,
...
__global__ void __launch_bounds__(NUM_THREADS)
    sgemmWarptiling(char *A, char *B, int *C, int M, int N, int K) {
  {
  ...
  int regM[WMITER * TM] = {0};
  int regN[WNITER * TN] = {0};
  ...
  for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx) {
    for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
      ...
      int *C_interim = C + (wSubRowIdx * WSUBM) * N + wSubColIdx * WSUBN;
