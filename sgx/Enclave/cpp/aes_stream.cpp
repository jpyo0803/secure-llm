...
static void _aes_key_expand_128(__m128i round_keys[AES_STREAM_ROUNDS + 1],
                                __m128i t1) {
  __m128i s;
...
static void _aes_key_expand_256(__m128i round_keys[AES_STREAM_ROUNDS + 1],
...
static void _aes_stream(_aes_stream_state *_st, unsigned char *buf,
                        size_t buf_len) {
...
#define COMPUTE_AES_STREAM_ROUNDS(N)                                   \
  do {                                                                 \
    ...
    r##N = _mm_aesenc_si128(_mm_aesenc_si128(r##N, round_keys[1]),     \
                            round_keys[2]);                            \
    r##N = _mm_aesenc_si128(_mm_aesenc_si128(r##N, round_keys[3]),     \
                            round_keys[4]);                            \
    s##N = r##N;                                                       \
    r##N = _mm_aesenc_si128(_mm_aesenc_si128(r##N, round_keys[5]),     \
                            round_keys[6]);                            \
    r##N = _mm_aesenc_si128(_mm_aesenc_si128(r##N, round_keys[7]),     \
                            round_keys[8]);                            \
    r##N = _mm_aesenclast_si128(_mm_aesenc_si128(r##N, round_keys[9]), \
                                round_keys[10]);                       \
...
#define COMPUTE_AES_STREAM_ROUNDS(N)                                    \
  do {                                                                  \
    ...
    r##N = _mm_aesenc_si128(_mm_aesenc_si128(r##N, round_keys[7]),      \
                            round_keys[8]);                             \
    r##N = _mm_aesenc_si128(_mm_aesenc_si128(r##N, round_keys[9]),      \
                            round_keys[10]);                            \
    r##N = _mm_aesenc_si128(_mm_aesenc_si128(r##N, round_keys[11]),     \
                            round_keys[12]);                            \
    r##N = _mm_aesenclast_si128(_mm_aesenc_si128(r##N, round_keys[13]), \
                                round_keys[14]);                        \
    r##N = _mm_xor_si128(s##N, r##N);                                   \
  } while (0)
#endif
...
void aes_stream_init(aes_stream_state *st,
  ...
  _aes_key_expand_256(
      _st->round_keys, _mm_loadu_si128((const __m128i *)(const void *)seed),
